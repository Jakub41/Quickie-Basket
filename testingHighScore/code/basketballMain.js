(function (library, img, cjs) {
		...
		var levelArray=[
		[10/*how much baskets per level*/,1/*player number ,possibility to add more players in afuture*/,4500/*level time millsec*/,true/*is bonus round available*/,10000/*  bonus round time milisec*/,6000/*milisecodns on how fast board moves from on side to another for currentlevel*/],
		...
		function startScreen(){
			...
			mainParentLibrray.basket.highlight.visible=false;
		...
		function checkHighScore(score) {
			...
			{
				top10List = localStorage.getItem("top10List");
				
				if (localStorage.getItem("top10List")==null) {
					top10List = userName + " - " + score;
					localStorage.setItem("top10List", top10List );
					 
				} else {
					var theArray = top10List.split("\n");
					theResult = "";
					theCounter = 0;
					isInserted = false;
					
					for (var i=0; i <theArray.length; i++) {
						var theScore = theArray[i] + "";
						theScore = theScore.substring( theArray[i].indexOf("-") +1 ) ;
						theScore.trim();
					
						if ( parseInt(theScore) < score && !isInserted) {
							if (theCounter < 10) {
								theResult = theResult + userName + " - " + score;
								isInserted = true;
								theCounter++;
								if (theCounter > 0) {
									theResult = theResult +  "\n" ;
						...
						if (theCounter < 10) {
							theResult = theResult + theArray[i];
							theCounter++;
						}
						if (theCounter > 0) {
							theResult = theResult +  "\n" ;
					...
					if (theCounter < 10 && !isInserted) {
						theResult = theResult + userName + " - " + score;
					}
					
					localStorage.setItem("top10List", theResult);
				}
				alert("TOP 10 HIGH SCORE:\n"+ localStorage.getItem("top10List") );
			}
			else
			{
				alert("Your browser doesn't support web Storage, high scores cannot be saved");
			}
